...
export default function Sidebar() {
    ...
    return (
        <div className="hidden lg:flex lg:flex-shrink-0">
            <div className="flex flex-col w-64 border-r border-slate-800 bg-slate-900 z-20 shadow-xl relative">
                {/* Branding */}
                <div className="flex h-16 shrink-0 items-center px-5 border-b border-slate-800/50">
                                ...
                                {group.items
                                    ...
                                    .map((navItem) => {
                                        ...
                                        return (
                                            ...
                                            >
                                                <navItem.icon
                                                    className={`flex-shrink-0 mr-3 h-4 w-4 transition-colors ${ isActive ? 'text-brand-400' : 'text-slate-500 group-hover:text-slate-300'}`}
                                                />
                                                {navItem.name}
                                            </Link>
                                        );
                ...
                <div className="flex-shrink-0 border-t border-slate-800/50 p-3">
                    <div className="flex items-center gap-3 px-2 py-2 rounded-xl hover:bg-slate-800/50 transition-colors group">
                        ...
                        <button
                            onClick={() => logout?.()}
                            title="Sign out"
                            className="p-1.5 rounded-lg text-slate-500 hover:text-rose-500 hover:bg-rose-900/20 transition-colors opacity-0 group-hover:opacity-100 shrink-0"
                        >
